...
namespace MonyLoop.Application.Services.UserAuth
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<Result> ForgotPasswordAsync(ForgotPasswordRequestDto request, CancellationToken ct = default)
        {
            ...
            if (user == null)
            {
                // حماية ضد Timing Attacks: محاكاة نفس وقت التنفيذ تقريباً عشان الهاكر مايلاحظش فرق في سرعة الاستجابة
                await Task.Delay(50, ct);
                return Result.Ok();
            }

            await _otpService.GenerateAndSendAsync(user.Id, user.Email!, user.FirstName, OTPPurpose.PasswordReset, ct);
            return Result.Ok();
        }
        ...
        public async Task<Result> ResetPasswordAsync(ResetPasswordRequestDto request, CancellationToken ct = default)
        {
            ...
            // حماية أمنية: عدم الإفصاح عما إذا كان الإيميل مسجلاً أم لا، وإعطاء رسالة خطأ عامة
            if (user == null)
            {
                await Task.Delay(50, ct);
                return Result.Fail(Error.Validation("OTP.InvalidCode", "الكود غير صحيح أو منتهي الصلاحية."));
            }

            var verifyResult = await _otpService.VerifyAsync(user.Id, request.Code, OTPPurpose.PasswordReset, ct);
            if (verifyResult.IsFailure)
                return verifyResult;
